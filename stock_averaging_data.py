...
if __name__ == "__main__":
    print("=== Web_03 물타기 계산기 API 테스트 ===\n")

    try:
        print("📌 모듈 구조 테스트")
        print("-" * 60)

        # AveragingCalculator 직접 테스트
        from averaging_calculator import AveragingCalculator
        ...
        print("✅ AveragingCalculator 동작 확인:")
        print(f"  - 입력: 평단가 75,000원, 보유 100주")
        print(f"  - 추가: 70,000원, 10주")
        print(f"  - 새 평단가: {test_result['new_avg']:,}원")
        print(f"  - 평단가 변화: {test_result['change']:,}원 ({test_result['change_pct']:.2f}%)")
        print()

        print("📌 StockAveragingDataProvider 초기화")
        print("-" * 60)
        provider = StockAveragingDataProvider()
        print("✅ StockAveragingDataProvider 초기화 성공")
        print()

        # 히스토리 저장 경로 확인
        print("📌 히스토리 저장 경로")
        print("-" * 60)
        print(f"✅ 저장 경로: {provider.history_base_path}")
        print(f"✅ 경로 존재: {provider.history_base_path.exists()}")
        print()
        ...
        print("✅ 모듈 구조 테스트 완료")
        print()
        print("⚠️  실제 API 테스트는 계좌 연동 후 진행하세요:")
        print("  1. KIS API 키가 .env에 올바르게 설정되어 있는지 확인")
        print("  2. KIS_ENV=prod (실전) 또는 vps (모의투자) 확인")
        print("  3. 보유 종목이 있는 상태에서 테스트")
        print()
        print("📝 사용 예시:")
